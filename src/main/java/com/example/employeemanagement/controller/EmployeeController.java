...
Employee updatedEmployee = employeeService.updateEmployee(id, employeeDetails);
  ...
  @Operation(summary = "Get current logged-in employee profile", description = "Retrieve profile of the current authenticated employee")
@GetMapping("/me")
public ResponseEntity<Employee> getMyProfile(org.springframework.security.core.Authentication authentication) {
    // Lấy thông tin user hiện tại từ SecurityContext thông qua Authentication
    com.example.employeemanagement.security.UserDetailsImpl principal = 
        (com.example.employeemanagement.security.UserDetailsImpl) authentication.getPrincipal();
    
    // Giả định userId của tài khoản trùng với EmployeeId như cấu trúc bạn đang dùng
    Long employeeId = principal.getId(); 

    Employee employee = employeeService.getEmployeeById(employeeId)
            .orElseThrow(() -> new ResourceNotFoundException("Không tìm thấy thông tin nhân viên cho user ID: " + employeeId));
